...
export function* onValidateBilanStart({ payload }) {
  const { id, decision } = payload;
  try {
    yield call(validateBilan, id, decision);
    yield put(validateBilanSuccess(id, decision.etat));
  } catch (error) {
    console.log(error);
  }
}
export function* watchValidateBilanStart() {
  yield takeLatest(BilanActions.START_VALIDATE_BILAN, onValidateBilanStart);
}
